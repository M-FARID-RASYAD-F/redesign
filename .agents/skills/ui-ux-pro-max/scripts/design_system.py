...
def _style_is_dark_primary(style: dict) -> bool:
    ...
    if (style.get("Light Mode âœ“") == "not-recommended"
            and style.get("Dark Mode âœ“") == "supported"):
        return True
    declared = "{} {}".format(
        style.get("Light Mode âœ“", ""), style.get("Dark Mode âœ“", "")
    ).lower()
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        return {
            ...
            "style": {
                ...
                "light_mode": best_style.get("Light Mode âœ“", ""),
                "dark_mode": best_style.get("Dark Mode âœ“", ""),
            },
            "colors": {
...
def hex_to_ansi(hex_color: str) -> str:
    """Convert hex color to ANSI True Color swatch (â–ˆâ–ˆ) with fallback."""
    ...
    return f"\033[38;2;{r};{g};{b}mâ–ˆâ–ˆ\033[0m "
...
def section_header(name: str, width: int) -> str:
    """Create a Unicode section separator: â”œâ”€â”€â”€ NAME â”€â”€â”€...â”¤"""
    label = f"â”€â”€â”€ {name} "
    fill = "â”€" * (width - len(label) - 1)
    return f"â”œ{label}{fill}â”¤"
...
def format_ascii_box(design_system: dict) -> str:
    ...
    lines.append(ansi_ljust(f"â•‘  TARGET: {project} - RECOMMENDED DESIGN SYSTEM", BOX_WIDTH) + "â•‘")
    ...
    if any(dials.get(k) is not None for k in ("variance", "motion", "density")):
        ...
        if dials.get("variance") is not None:
            lines.append(f"â”‚  Variance: {dials['variance']}/10 â€” {dials['variance_label']}".ljust(BOX_WIDTH) + "â”‚")
        if dials.get("motion") is not None:
            lines.append(f"â”‚  Motion:   {dials['motion']}/10 â€” {dials['motion_label']}".ljust(BOX_WIDTH) + "â”‚")
        if dials.get("density") is not None:
            lines.append(f"â”‚  Density:  {dials['density']}/10 â€” {dials['density_label']}".ljust(BOX_WIDTH) + "â”‚")

    # Pattern section
    lines.append(section_header("PATTERN", BOX_WIDTH + 1))
    lines.append(f"â”‚  Name: {pattern.get('name', '')}".ljust(BOX_WIDTH) + "â”‚")
    if pattern.get('conversion'):
        lines.append(f"â”‚     Conversion: {pattern.get('conversion', '')}".ljust(BOX_WIDTH) + "â”‚")
    if pattern.get('cta_placement'):
        lines.append(f"â”‚     CTA: {pattern.get('cta_placement', '')}".ljust(BOX_WIDTH) + "â”‚")
    lines.append("â”‚     Sections:".ljust(BOX_WIDTH) + "â”‚")
    for i, section in enumerate(sections, 1):
        lines.append(f"â”‚       {i}. {section}".ljust(BOX_WIDTH) + "â”‚")

    # Style section
    lines.append(section_header("STYLE", BOX_WIDTH + 1))
    lines.append(f"â”‚  Name: {style.get('name', '')}".ljust(BOX_WIDTH) + "â”‚")
    light = style.get("light_mode", "")
    dark = style.get("dark_mode", "")
    if light or dark:
        lines.append(f"â”‚     Mode Support: Light {light}  Dark {dark}".ljust(BOX_WIDTH) + "â”‚")
    if style.get("keywords"):
        for line in wrap_text(f"Keywords: {style.get('keywords', '')}", "â”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "â”‚")
    if style.get("best_for"):
        for line in wrap_text(f"Best For: {style.get('best_for', '')}", "â”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "â”‚")
    if style.get("performance") or style.get("accessibility"):
        perf_a11y = f"Performance: {style.get('performance', '')} | Accessibility: {style.get('accessibility', '')}"
        lines.append(f"â”‚     {perf_a11y}".ljust(BOX_WIDTH) + "â”‚")
    ...
    for label, key, css_var in SEMANTIC_COLOR_ENTRIES:
        ...
        content = f"â”‚     {swatch}{label + ':':14s} {hex_val:10s} ({css_var})"
        lines.append(ansi_ljust(content, BOX_WIDTH) + "â”‚")
    if colors.get("notes"):
        for line in wrap_text(f"Notes: {colors.get('notes', '')}", "â”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "â”‚")

    # Typography section
    lines.append(section_header("TYPOGRAPHY", BOX_WIDTH + 1))
    lines.append(f"â”‚  {typography.get('heading', '')} / {typography.get('body', '')}".ljust(BOX_WIDTH) + "â”‚")
    if typography.get("mood"):
        for line in wrap_text(f"Mood: {typography.get('mood', '')}", "â”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "â”‚")
    if typography.get("best_for"):
        for line in wrap_text(f"Best For: {typography.get('best_for', '')}", "â”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "â”‚")
    if typography.get("google_fonts_url"):
        lines.append(f"â”‚     Google Fonts: {typography.get('google_fonts_url', '')}".ljust(BOX_WIDTH) + "â”‚")
    if typography.get("css_import"):
        lines.append(f"â”‚     CSS Import: {typography.get('css_import', '')[:70]}...".ljust(BOX_WIDTH) + "â”‚")

    # Key Effects section
    if effects:
        lines.append(section_header("KEY EFFECTS", BOX_WIDTH + 1))
        for line in wrap_text(effects, "â”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "â”‚")

    # Motion section (GSAP skeleton, only if --motion dial was set)
    if motion_snippet:
        lines.append(section_header("MOTION", BOX_WIDTH + 1))
        lines.append(f"â”‚  {motion_snippet.get('Category', '')} ({motion_snippet.get('Intensity Tier', '')})".ljust(BOX_WIDTH) + "â”‚")
        lines.append(f"â”‚     Trigger: {motion_snippet.get('Trigger', '')} | Duration: {motion_snippet.get('Duration', '')} | Easing: {motion_snippet.get('Easing', '')}".ljust(BOX_WIDTH) + "â”‚")
        for line in wrap_text(f"GSAP: {motion_snippet.get('GSAP Snippet', '')}", "â”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "â”‚")
        if motion_snippet.get("Framework Notes"):
            for line in wrap_text(f"Framework: {motion_snippet.get('Framework Notes', '')}", "â”‚     ", BOX_WIDTH):
                lines.append(line.ljust(BOX_WIDTH) + "â”‚")

    # Anti-patterns section
    if anti_patterns:
        lines.append(section_header("AVOID", BOX_WIDTH + 1))
        for line in wrap_text(anti_patterns, "â”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "â”‚")
    ...
    for item in checklist_items:
        lines.append(f"â”‚     {item}".ljust(BOX_WIDTH) + "â”‚")
...
def format_markdown(design_system: dict) -> str:
    ...
    if any(dials.get(k) is not None for k in ("variance", "motion", "density")):
        ...
        if dials.get("variance") is not None:
            lines.append(f"- **Variance:** {dials['variance']}/10 â€” {dials['variance_label']}")
        if dials.get("motion") is not None:
            lines.append(f"- **Motion:** {dials['motion']}/10 â€” {dials['motion_label']}")
        if dials.get("density") is not None:
            lines.append(f"- **Density:** {dials['density']}/10 â€” {dials['density_label']}")
        lines.append("")
    ...
    if motion_snippet:
        lines.append("### Motion")
        lines.append(f"**{motion_snippet.get('Category', '')}** ({motion_snippet.get('Intensity Tier', '')}) â€” Trigger: {motion_snippet.get('Trigger', '')} | Duration: {motion_snippet.get('Duration', '')} | Easing: `{motion_snippet.get('Easing', '')}`")
        lines.append("```js")
        ...
        if motion_do:
            lines.append(f"- âœ… {motion_do}")
        if motion_dont:
            lines.append(f"- âŒ {motion_dont}")
        lines.append("")
...
def safe_slug(name, fallback: str = "default") -> str:
    ...
    (e.g. "../../etc") impossible â€” the slug can never leave its parent dir.
    """
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("")
    if spacing_scale:
        lines.append(f"*Density: {dials.get('density')}/10 â€” {dials.get('density_label')}*")
        lines.append("")
    ...
    if motion_snippet:
        ...
        if motion_snippet.get("Performance Notes"):
            lines.append(f"- âš¡ {motion_snippet.get('Performance Notes', '')}")
        lines.append("")
    ...
    if anti_patterns:
        ...
        for anti in anti_list:
            if anti:
                lines.append(f"- âŒ {anti}")
    lines.append("")
    lines.append("### Additional Forbidden Patterns")
    lines.append("")
    lines.append("- âŒ **Emojis as icons** â€” Use SVG icons (Heroicons, Lucide, Simple Icons)")
    lines.append("- âŒ **Missing cursor:pointer** â€” All clickable elements must have cursor:pointer")
    lines.append("- âŒ **Layout-shifting hovers** â€” Avoid scale transforms that shift layout")
    lines.append("- âŒ **Low contrast text** â€” Maintain 4.5:1 minimum contrast ratio")
    lines.append("- âŒ **Instant state changes** â€” Always use transitions (150-300ms)")
    lines.append("- âŒ **Invisible focus states** â€” Focus states must be visible for a11y")
    lines.append("")
...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    if spacing:
        ...
        lines.append("- No overrides â€” use Master spacing")
    lines.append("")
    ...
    if typography:
        ...
        lines.append("- No overrides â€” use Master typography")
    lines.append("")
    ...
    if colors:
        ...
        lines.append("- No overrides â€” use Master colors")
    lines.append("")
    ...
    if components:
        ...
        lines.append("- No overrides â€” use Master component specs")
    lines.append("")
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    if style_results:
        ...
        if any(kw in keywords.lower() for kw in ["data", "dense", "dashboard", "grid"]):
            ...
            spacing["Content Density"] = "High â€” optimize for information display"
            ...
            spacing["Content Density"] = "Low â€” focus on clarity"
        else:
