        ...
        if args.json:
            ...
            if args.persist:
                ...
                if persistence.get("status") == "skipped_exists":
                    print(f"âš ï¸  {persistence.get('message', 'MASTER.md already exists; not overwritten.')}")
                else:
                    ds_dir = persistence.get("design_system_dir", "design-system/<project>")
                    print(f"âœ… Design system persisted to {ds_dir}/")
                    for f in persistence.get("created_files", []):
                        print(f"   ðŸ“„ {f}")
                    print("")
                    print(f"ðŸ“– Usage: When building a page, check {ds_dir}/pages/[page].md first.")
