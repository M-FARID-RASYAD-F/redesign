...
<div class="perpus-page-wrapper">
    <div class="perpus-container-medium">
        ...
        <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
            <a href="{{ route('perpus.index') }}" class="perpus-btn-detail inline-flex items-center gap-2 px-4 py-2.5 text-sm">
            ...
            <div class="perpus-detail-grid">
                ...
                <div class="p-6 sm:p-8 flex flex-col justify-between relative overflow-hidden perpus-card-subsurface">
                    ...
                    <div class="pt-5 border-t border-white/15">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-3 h-3 rounded-full {{ $book->available > 0 ? 'bg-emerald-400 animate-pulse' : 'bg-rose-500' }}"></div>
                        ...
                        <div class="text-xs text-slate-400 mt-1">Total Koleksi Sekolah: {{ $book->stock }} eksemplar</div>
                    </div>
                </div>
                ...
                <div class="p-6 sm:p-10 flex flex-col justify-between">
                    <div>
                        ...
                        <!-- Metadata Grid -->
                        <div class="grid grid-cols-2 sm:grid-cols-2 gap-4 py-4 border-y border-white/10 text-sm mb-6">
                            <div class="p-3 rounded-xl bg-white/5 border border-white/10">
                                <span class="text-slate-400 block text-xs mb-1">Penulis / Pengarang</span>
                                <span class="font-semibold text-white text-sm sm:text-base">{{ $book->author }}</span>
                            </div>
                            <div class="p-3 rounded-xl bg-white/5 border border-white/10">
                                <span class="text-slate-400 block text-xs mb-1">Penerbit</span>
                                <span class="font-semibold text-white text-sm sm:text-base">{{ $book->publisher ?: '-' }}</span>
                            </div>
                            <div class="p-3 rounded-xl bg-white/5 border border-white/10">
                                <span class="text-slate-400 block text-xs mb-1">Nomor ISBN</span>
                                <span class="font-mono text-cyan-300 text-xs sm:text-sm font-semibold">{{ $book->isbn ?: 'Tidak Terdaftar' }}</span>
                            </div>
                            <div class="p-3 rounded-xl bg-white/5 border border-white/10">
                        ...
                        <div>
                            ...
                            <div class="text-slate-300 text-sm sm:text-base leading-relaxed whitespace-pre-line bg-white/5 p-4 rounded-xl border border-white/5">
                    ...
                    <div class="mt-8 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center gap-4">
                        @if($book->available > 0)
                        <a href="{{ route('perpus.pinjam.create', $book->id) }}" class="perpus-btn-borrow w-full sm:w-auto flex-1 text-center py-4 px-6 text-base font-bold">
                        ...
                        @else
                        <div class="w-full sm:w-auto flex-1 py-3.5 px-6 perpus-btn-disabled text-center font-bold">
                            Seluruh Eksemplar Sedang Dipinjam
                        </div>
                        @endif
                        <a href="{{ route('perpus.index') }}" class="perpus-btn-detail w-full sm:w-auto px-6 py-4 text-center text-sm font-semibold">
                            Cari Buku Lain
                        </a>
                    </div>
                </div>
